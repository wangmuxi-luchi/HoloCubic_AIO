...
{
    // ARGB8888 → RGB565: R5=bits[19:15], G6=bits[10:5], B5=bits[4:0]
    uint16_t c = (uint16_t)(((rgb & 0xF80000) >> 8) | ((rgb & 0xFC00) >> 5) | ((rgb & 0xFF) >> 3));
...
{
    screen_buf = (uint16_t *)malloc(SCREEN_HEIGHT * SCREEN_WIDTH * sizeof(uint16_t)); //动态分配一块屏幕分辨率大小的空间
    if (screen_buf == NULL)
